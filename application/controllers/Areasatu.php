...
class Areasatu extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        // is_login();
        $this->load->model('Areasatu_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            ...
            $config['first_url'] = base_url() . 'index.php/areasatu/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/areasatu/index/';
            $config['first_url'] = base_url() . 'index.php/areasatu/index/';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = FALSE;
        $config['total_rows'] = $this->Areasatu_model->total_rows($q);
        $areasatu = $this->Areasatu_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $data = array(
            'areasatu_data' => $areasatu,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->template->load('template','areasatu/tbl_pemenuhansatu_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Areasatu_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'nama_file' => $row->nama_file,
            ...
            $this->template->load('template','areasatu/tbl_pemenuhansatu_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('areasatu'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('areasatu/create_action'),
        ...
        $this->template->load('template','areasatu/tbl_pemenuhansatu_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'nama_file' => $this->input->post('nama_file',TRUE),
            ...
            $this->Areasatu_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('areasatu/update_action'),
    ...
    public function update_action() 
    {
		...
		'nama_file' => $this->input->post('nama_file',TRUE),
            ...
            $this->Areasatu_model->update($this->input->post('id_pemenuhansatu', TRUE), $data);
	...
	foreach ($this->Areasatu_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'tbl_pemenuhansatu_data' => $this->Areasatu_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('areasatu/tbl_pemenuhansatu_doc',$data);
    }

}

/* End of file Areasatu.php */
/* Location: ./application/controllers/Areasatu.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2022-07-19 05:23:23 */
/* http://harviacode.com */